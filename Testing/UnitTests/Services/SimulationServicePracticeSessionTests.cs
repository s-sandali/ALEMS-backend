...
public class SimulationServicePracticeSessionTests
{
    ...
    [Fact]
    public async Task ValidateStepAsync_SelectionSortWrongSelectMinIndex_ReturnsIncorrectAndDoesNotAdvance()
    {
        var store = new InMemorySimulationSessionStore();
        var sut = BuildSelectionSortSut(store);
        ...
        var beforeInvalidStep = store.Get(session.SessionId)!;
        var invalidResponse = await sut.ValidateStepAsync(session.SessionId, "select_min", [2]);

        invalidResponse.Correct.Should().BeFalse();
        invalidResponse.NextExpectedAction.Should().Be("select_min");
        invalidResponse.SuggestedIndices.Should().Equal([1]);
        invalidResponse.Message.Should().Be("Incorrect step.");

        var afterInvalidStep = store.Get(session.SessionId)!;
        afterInvalidStep.CurrentStepIndex.Should().Be(beforeInvalidStep.CurrentStepIndex);
    }

    [Fact]
    public async Task ValidateStepAsync_SelectionSortWhenSessionIsTerminal_ReturnsPracticeCompleteWithoutAdvancing()
    {
        var store = new InMemorySimulationSessionStore();
        var sut = BuildSelectionSortSut(store);

        var session = await sut.StartSessionAsync("selection_sort", [2, 1], null);
        ...
        var selectMinResponse = await sut.ValidateStepAsync(session.SessionId, "select_min", [1]);
        selectMinResponse.Correct.Should().BeTrue();
        selectMinResponse.NextExpectedAction.Should().Be("swap");

        var swapResponse = await sut.ValidateStepAsync(session.SessionId, "swap", [0, 1]);
        swapResponse.Correct.Should().BeTrue();
        swapResponse.NextExpectedAction.Should().Be("complete");

        var beforeTerminalValidation = store.Get(session.SessionId)!;
        var terminalResponse = await sut.ValidateStepAsync(session.SessionId, "compare", [0, 1]);

        terminalResponse.Correct.Should().BeFalse();
        terminalResponse.NextExpectedAction.Should().Be("complete");
        terminalResponse.Message.Should().Be("Practice complete.");
        terminalResponse.Hint.Should().Be("No more actions are needed.");
        terminalResponse.SuggestedIndices.Should().BeEmpty();

        var afterTerminalValidation = store.Get(session.SessionId)!;
        afterTerminalValidation.CurrentStepIndex.Should().Be(beforeTerminalValidation.CurrentStepIndex);
    }

    [Fact]
    public async Task ValidateStepAsync_SelectionSortWhenCurrentIndexIsNonInteractive_NormalizesToNextInteractiveStep()
    {
        var store = new InMemorySimulationSessionStore();
        var sut = BuildSelectionSortSut(store);

        var session = await sut.StartSessionAsync("selection_sort", [64, 25, 12, 22, 11], null);
        var persisted = store.Get(session.SessionId)!;

        // Force an invalid state to verify NormalizeSessionIndex behavior.
        persisted.CurrentStepIndex = 0;

        var response = await sut.ValidateStepAsync(session.SessionId, "compare", [0, 1]);

        response.Correct.Should().BeTrue();
        response.NextExpectedAction.Should().Be("select_min");
        response.SuggestedIndices.Should().Equal([1]);

        var updated = store.Get(session.SessionId)!;
        updated.CurrentStepIndex.Should().Be(response.CurrentStepIndex);
        updated.CurrentStepIndex.Should().BeGreaterThan(0);
    }
